-- 1) Fix ingest_strava_activity: only attach to a registration that actually matched.
...
-- 2) Reconcile existing mis-attributed activity_logs.
-- Detach Strava logs whose sport does not match the registration's activity_mode
-- (only when the registration is mode-restricted: run/walk/ride).
WITH bad AS (
  SELECT a.id, a.registration_id
  FROM public.activity_logs a
  JOIN public.registrations r ON r.id = a.registration_id
  WHERE r.activity_mode::text IN ('run','walk','ride')
    AND lower(COALESCE(a.sport_type, a.activity_type)) <> ALL (
      CASE r.activity_mode::text
        WHEN 'run'  THEN ARRAY['run','virtualrun','trailrun']
        WHEN 'walk' THEN ARRAY['walk','hike']
        WHEN 'ride' THEN ARRAY['ride','virtualride','ebikeride','mountainbikeride','gravelride','cycling']
      END
...
affected AS (
  SELECT DISTINCT registration_id FROM bad WHERE registration_id IS NOT NULL
),
upd AS (
  UPDATE public.activity_logs a SET registration_id = NULL
  WHERE a.id IN (SELECT id FROM bad)
  RETURNING 1
)
UPDATE public.registrations r
   SET total_km_logged = public._registration_logged_km(r.id)
 WHERE r.id IN (SELECT registration_id FROM affected);
