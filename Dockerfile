...
# Placeholders only. In backend mode nothing calls Supabase, but the SDK
# throws at import time if these are empty — which blanks the whole SPA.
ARG VITE_SUPABASE_URL="https://placeholder.supabase.co"
ARG VITE_SUPABASE_PUBLISHABLE_KEY="placeholder-anon-key"
ARG VITE_SUPABASE_PROJECT_ID="placeholder"
ENV VITE_BACKEND_ENABLED=$VITE_BACKEND_ENABLED \
    ...
    VITE_SUPABASE_PUBLISHABLE_KEY=${VITE_SUPABASE_PUBLISHABLE_KEY:-placeholder-anon-key} \
